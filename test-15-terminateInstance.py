...
class TestVerifyTerminateInstance(unittest.TestCase):
    def setUp(self):
        sel_setup = SelSetup()
        sel_setup.desired_capabilities["name"] = "Terminate Instance"
    ...
    def test_verify_terminate_instance(self):
        self.retry = 20
        self.passed = False
        driver = self.driver
        ...
        for i in range(self.retry):
            if self.is_element_present(By.LINK_TEXT, "Terminate"):
                break
            time.sleep(1)
            if i == (self.retry - 1):
                self.verificationErrors.append("Could not find 'Terminate' link")
                return False
        ...
        try: self.assertRegexpMatches(driver.find_element_by_css_selector("BODY").text, r"^[\s\S]*There are no instances that match your criteria[\s\S]*$")
